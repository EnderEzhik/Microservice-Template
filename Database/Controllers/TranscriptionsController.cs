...
public class TranscriptionsController : ControllerBase
{
    ...
    {
        logger.Information("GET request for get transcription with url: {Url}", url);
        ...
        {
            logger.Error("An error occurred while getting a transcription with url: {Url}", url);
            return Problem(ex.Message, nameof(this.GetTranscription), 500, $"An error occurred while getting a transcription with url: {url}");
        }
    }

    [HttpPost]
    public async Task<ActionResult<Transcription>> SaveTranscription(
        TranscriptionService transcriptionService, Transcription transcription)
    {
        ...
        try
        {
            ...
            logger.Information("Transcription saved successfully with url: {Url}", transcription.Url);
            return CreatedAtAction(nameof(GetTranscription), new { Url = transcription.Url }, transcription);
        }
        catch (Exception ex)
        {
            logger.Error(ex, "An error occurred while saving a transcription with url: {Url}", transcription.Url);
            return Problem(ex.Message, nameof(this.SaveTranscription), 500,  "An error occurred while saving a transcription with url: {Url}");
        }
    }
}
